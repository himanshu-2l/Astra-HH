...
class EnsembleRetriever:
    def __init__(self, embed_device: str = "cuda:1", rerank_device: str = "cuda:1"):
        ...
        print("✅ Ensemble Retriever fully loaded and primed in RAM!")
    ...
    def search(self, query: str, top_k: int = 5) -> Dict[str, Any]:
        ...
        def search_strategy(strategy):
            # Dense Search
            dense_scores, dense_ids = self.faiss_mgr.search_strategy(strategy, query_emb, top_k=50)
            dense_results = [self.chunk_lookups[strategy][idx] for idx in dense_ids if idx < len(self.chunk_lookups[strategy])]
            
            # Sparse Search
            sparse_ids, sparse_scores = self.bm25_mgr.search_strategy(strategy, query, top_k=50)
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            results = list(executor.map(search_strategy, STRATEGIES))
        ...
        fused_candidates = reciprocal_rank_fusion(ranked_lists, k=60, top_n=50)
        t_fuse = (time.perf_counter() - t_fuse_start) * 1000
        
        # 4. Cross-Encoder Re-ranking
        t_rerank_start = time.perf_counter()
        top_chunks = self.reranker.rerank(query, fused_candidates, top_k=top_k)
