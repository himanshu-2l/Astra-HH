...
const FRAGMENT_SHADER = `
  ...
  // Color modified for blue/white/cosmic palette (extracted from flow.stephenhung.me)
  ...
  void main() {
    ...
    // Deep blue / violet tint with glowing white highlights
    vec3 blue = vec3(0.08, 0.12, 0.25) + luma * vec3(0.7, 0.8, 1.0);
...
export const FlowCanvas: React.FC<FlowCanvasProps> = ({ scale = 1 }) => {
  ...
  useEffect(() => {
    ...
    const vs = createShader(VERTEX_SHADER, gl.VERTEX_SHADER);
    const fs = createShader(FRAGMENT_SHADER, gl.FRAGMENT_SHADER);
    if (!vs || !fs) return;
